...
export async function buildServer(): Promise<FastifyInstance> {
  ...
  app.get("/v1/daily-luck", async (req: FastifyRequest<{ Querystring: { date?: string } }>, reply: FastifyReply) => {
  ...
  app.post("/v1/fortune/draw", async (req: FastifyRequest<{ Body: { at?: string } }>, reply: FastifyReply) => {
    const at = req.body?.at ? new Date(req.body.at) : new Date();
    if (Number.isNaN(at.getTime())) return reply.code(400).send({ error: "Invalid at" });
    return drawFortune(config, at);
  });

  app.post(
    "/v1/decision/yesno",
    async (req: FastifyRequest<{ Body: { question?: string; at?: string } }>, reply: FastifyReply) => {
      const at = req.body?.at ? new Date(req.body.at) : new Date();
      if (Number.isNaN(at.getTime())) return reply.code(400).send({ error: "Invalid at" });
